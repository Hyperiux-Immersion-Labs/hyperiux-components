...
/**
 * BorderBeam styles - presets, color palettes, and per-instance CSS generation.
 *
...
const colorPalettes = {
  colorful: {
    border: [
      ...
      { color: 'rgb(180, 40, 240)', pos: '55% 0%', size: '160px 72px' },
      { color: 'rgb(255, 90, 70)', pos: '74% 1%', size: '160px 72px' },
...
/**
 * @param {any} colorVariant
 * @returns {string}
 */
function getSmallColorGradients(colorVariant) {
  const palette = smallColorPalettes[colorVariant];
  return palette.border
    .map((/** @type {any} */ c) => `radial-gradient(ellipse ${c.size} at ${c.pos}, ${c.color}, transparent)`)
    .join(',\n    ');
}
...
function getInnerGradients(colorVariant) {
  ...
  return palette.border
    .map((/** @type {any} */ c) => {
      ...
      const smallerSize = c.size.split(' ').map((/** @type {any} */ s) => {
...
function getLineBloomGradients(colorVariant, isDark, id) {
  ...
  const spikes = bloomData.spikes.map((/** @type {any} */ s) => isMono
...
function pulseInnerGradients(variant, id, isDark) {
  ...
  const grads = palette.map((/** @type {any} */ c, /** @type {number} */ i) => {
    const { region, quad } = PULSE_RING_MAP[i];
    const [x, y] = c.pos.split(' ');
...
@media (prefers-reduced-motion: reduce) {
  [data-beam="${id}"] {
    --beam-motion-scale: ${REDUCED_MOTION_SCALE};
  }
}`;
}
...
function motionDur(seconds) {
  return `calc(${seconds}s * var(--beam-motion-scale, 1))`;
}
 ...
 * @param {boolean} staticColors
 * @param {string} id
 * @returns {{
 *   oscillators: { prop: string, a: number, b: number, period: number, delay: number, unit: string }[],
 *   hue: { prop: string, range: number, period: number, continuous: boolean } | null,
 * } | null}
 */
export function getPulseDriverConfig(size, theme, duration, hueRange, staticColors, id) {
 ...
 * @property {number} bloomOpacity
 * @property {string} innerShadow
 * @property {string} size
 * @property {string} colorVariant
 * @property {boolean} staticColors
 * @property {number} brightness
 * @property {number} saturation
 * @property {number} hueRange
 * @property {string} theme
 * @property {number} [hairlineOpacity]
 */

/**
 * Generate complete CSS for a BorderBeam instance
 * @param {BeamCSSOptions} options
 * @returns {string}
 */
export function generateBeamCSS(options) {
  const { size } = options;
...
function generateSmallVariantCSS(options) {
  ...
  const hueShiftAnimation = staticColors
    ? ''
    : `animation: beam-hue-shift-${id} ${motionDur(12)} ease-in-out infinite;`;
...
[data-beam="${id}"][data-active] {
  animation:
    beam-spin-${id} ${motionDur(duration)} linear infinite,
    beam-fade-in-${id} ${motionDur(0.6)} ease forwards;
}

[data-beam="${id}"][data-fading] {
  animation:
    beam-spin-${id} ${motionDur(duration)} linear infinite,
    beam-fade-out-${id} ${motionDur(0.5)} ease forwards;
}
...
function generateBorderVariantCSS(options) {
  ...
  const hueShiftKeyframes = staticColors ? '' : `
@keyframes beam-hue-shift-${id} {
  0% { filter: blur(9px) hue-rotate(calc(var(--beam-hue-base, 0deg) - ${hueRange}deg)) brightness(${brightness.toFixed(2)}) saturate(${saturation.toFixed(2)}); }
  50% { filter: blur(9px) hue-rotate(calc(var(--beam-hue-base, 0deg) + ${hueRange}deg)) brightness(${brightness.toFixed(2)}) saturate(${saturation.toFixed(2)}); }
  100% { filter: blur(9px) hue-rotate(calc(var(--beam-hue-base, 0deg) - ${hueRange}deg)) brightness(${brightness.toFixed(2)}) saturate(${saturation.toFixed(2)}); }
}`;
...
function generatePulseInnerVariantCSS(options) {
  ...
  // (see getPulseDriverConfig / pulseDriver.js); only `op` is needed here to set
...
function generatePulseOuterVariantCSS(options) {
  ...
  // (getPulseDriverConfig / pulseDriver.js); only `op` is needed here for the
...
function generateLineVariantCSS(options) {
  ...
  const hueShiftBloomAnimation = staticColors
    ? ''
    : `animation: beam-hue-shift-bloom-${id} ${motionDur(8)} ease-in-out infinite;`;
...
[data-beam="${id}"][data-active] {
  animation:
    beam-travel-${id} ${motionDur(duration)} linear infinite,
    beam-edge-fade-${id} ${motionDur(duration)} linear infinite,
    beam-breathe-${id} ${motionDur((duration * 1.3).toFixed(1))} ease-in-out infinite,
    beam-spike-${id} ${motionDur((duration * 1.33).toFixed(1))} ease-in-out infinite,
    beam-spike2-${id} ${motionDur((duration * 1.7).toFixed(1))} ease-in-out infinite,
    beam-fade-in-${id} ${motionDur(0.6)} ease forwards;
}
