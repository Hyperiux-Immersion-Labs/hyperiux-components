...
type RectangularTextRevealDirection = 'left' | 'right' | 'top' | 'bottom';

function getOrigins(direction: RectangularTextRevealDirection): { enterOrigin: string, exitOrigin: string, axis: 'scaleX' | 'scaleY' } {
  switch (direction) {
    case "right":
      return {
        enterOrigin: "100% 50%",
...
interface CreateRevealRectOptions {
  insetX: string;
  insetY: string;
  background: string;
...
function createRevealRect({
  ...
  zIndex,
  dataAttribute,
}: CreateRevealRectOptions): HTMLDivElement {
...
interface RectangularTextRevealProps {
  ...
  triggerStart?: string;
  once?: boolean;
  direction?: RectangularTextRevealDirection;
  delay?: number;
...
export default function RectangularTextReveal({
  ...
  useLayoutEffect(() => {
    ...
    lines.forEach((line) => {
      ...
      (line.parentNode as ParentNode).insertBefore(wrapper, line);
